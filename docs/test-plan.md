...
| TC-03 | list_jobs | jobs.json contains saved jobs | { "limit": 10 } | Returns saved jobs with correct format | |![alt text](image-4.png) |
| TC-04 | list_jobs | jobs.json contains many records | { "limit": 100000 } | Request rejected because limit exceeds maximum allowed value | |![alt text](image-1.png) |
| TC-05 | list_jobs | jobs.json is empty | {} | Returns empty jobs list without error | |![alt text](image-5.png) |
